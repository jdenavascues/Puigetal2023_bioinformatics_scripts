---
title: "II. Visualisation of DESeq2 results"
...
vsd <- readRDS(file.path('output', 'vst_pseudocounts_batchCorrected.RDS'))
```
...
# gene symbols
dlist <- read.table(file = file.path("resources", "gene_symbols.txt"), header=TRUE)
...
downplot <- plot(downfit,
                 ...
                 quantities = TRUE)
# save it
pdf(file=file.path(figdir, 'euler_up.pdf'),
     width = 10, height = 12)
upplot
dev.off()

pdf(file = file.path(figdir,'euler_down.pdf'),
    width = 10, height = 12)
downplot
dev.off()
...
genes_of_interest <- read_excel(file.path('resources', 'genes_of_interest.xlsx'),
                                sheet='gene_table') %>%
...
write_xlsx(unnest_longer(pmg.markers, xmarkers) %>%
    ...
    path = file.path('output', 'preliminary_Table S4.xlsx'))
